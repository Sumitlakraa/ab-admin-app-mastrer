...
import { BusLedgerDetail } from 'src/app/model/ledger';
...
export class BusLedgerComponent implements OnInit {
  ...
  displayedColumns: string[] = ['bus_number', 'tickets', 'amount', 'discount', 'commission', 'payable'];
  ledgerDetails: BusLedgerDetail[] = [];
  ...
  onViewLedgerDetailClick(ledgerDetail: BusLedgerDetail) {
    this.router.navigateByUrl('/dashboard/ticketLedger', { state: ledgerDetail });
  }
  ...
  convertJsonToObject(item: any) {
    let ledgerDetail: BusLedgerDetail = {
      id : '---'
    };
